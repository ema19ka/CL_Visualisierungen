...
//Areachart
async function drawAreaChart(url) {
  ...
  console.table(dataset[80]);
  const yAccessor = d => d.AnzahlFaelle; 
  const dateParser = d3.timeParse('%x');
...
/*-------------------------------------------------------------*/
  ...
  d3.select("#wrapper").call(hover);
    ...
    focus.append("text") //text dfinieren
      ...
      //bisect = d3.bisector(yAccessor(dataset)); 
      //console.log(dataset.taeglicheErkrankungen[x]); 
      ...
      // //console.log(d.taeglicheErkrankungen); 
